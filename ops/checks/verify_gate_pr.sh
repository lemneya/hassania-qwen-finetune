...
if git diff --name-only origin/main...HEAD 2>/dev/null | grep -q "^gates/"; then
  echo "Gate files modified — checking structure..."
  for gate_file in $(git diff --name-only origin/main...HEAD 2>/dev/null | grep "^gates/" | grep -v GATE_TEMPLATE.md); do
    if [[ -f "$gate_file" ]]; then
